...
@interface GLViewController (){
    ...
    // フレームバッファ
    //============
    GLint _def_frame_buffer;//IOSのデフォルトのフレームバッファ
...
- (void)setupGL
{
    ...
    //生成できたデフォルトのフレームバッファを取得する
    glGetIntegerv(GL_FRAMEBUFFER_BINDING, &_def_frame_buffer);
...
-(void)renderingBox{
    ...
    //(射影 x view) x ワールド加工系
    mat4* rotate = [mat4 rotate:[[vec3 alloc] init:0 y:1 z:0] radian:rrrr];
    rrrr += 0.01;
    
    //mat4* transform = [mat4 translate:[[vec3 alloc] init:x * 3 y:0 z:z * 3]];
    //mat4* matdata = [mat1 multiplyMat4:rotate];
    mat4* aaa = [mat4 translate:[[vec3 alloc] init:0 y:0 z:0]];
    
    verts* boxvert = [[verts alloc] init];
    [boxvert addVert:-0.5 y:0.5 z:0 u:1 v:1]; //右上
    [boxvert addVert:-0.5 y:-0.5 z:0 u:1 v:1]; //右下
    [boxvert addVert:0.5 y:0.5 z:0 u:1 v:1]; //左上
    [boxvert addVert:0.5 y:-0.5 z:0 u:1 v:1]; //左下
    
    [boxvert addTriangle:0 b:1 c:2];
    [boxvert addTriangle:2 b:1 c:3];
    [boxvert addTriangle:3 b:1 c:0];
    [boxvert addTriangle:0 b:2 c:3];
    ...
    glVertexAttribPointer(_attr_pos, 3, GL_FLOAT, GL_FALSE, sizeof(vertraw), (GLvoid*)posTri);
    ...
    glDrawElements(GL_TRIANGLES, 12, GL_UNSIGNED_SHORT, (GLvoid*)posIdx);
